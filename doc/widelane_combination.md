...
or since $\lambda=\frac{c}{f}$

$$
f_1 - f_2 = f_{WL}
$$

The following table presents GPS frequency and wavelength parameters for common widelane combinations (defined by frequency difference $f_1 - f_2$). Note that the widelane wavelength is significantly longer than the individual carrier wavelengths, which facilitates ambiguity resolution by reducing the search space.
...
$$
\lambda_{WL} (\Phi_1 - \Phi_2) = \frac{f_1}{f_1-f_2} L_1 - \frac{f_2}{f_1-f_2} L_2
$$
...
The narrow-lane combination uses the frequency sum to produce a shorter wavelength, which provides improved precision for fixing the widelane ambiguity.
