...
@contextmanager
def _goodstyle_context(enabled: bool):
    """Context manager that applies the light × minimal rcParams when enabled."""
    if enabled:
        with plt.rc_context(_GOODSTYLE_RC):
            yield
    else:
        yield


def _apply_goodstyle_axes(ax):
    """Remove top/right spines and style left/bottom spines for a single Axes when goodstyle is active."""
...
def plot_satellite_observations(
    ...
    Args:
        ...
        goodstyle: When True, apply a light × minimal visual style
    """
    ...
    for sat_id, data in satellite_data.items():
        ...
        with _goodstyle_context(goodstyle):
            _plot_single_satellite(
                sat_id,
                data,
                output_dir,
                plot_mode,
                show_ambiguity_statistics,
                goodstyle,
            )


def _plot_single_satellite(
    sat_id: str,
    data: dict,
    output_dir: Path,
    plot_mode: int,
    show_ambiguity_statistics: bool,
    goodstyle: bool,
):
